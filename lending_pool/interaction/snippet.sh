...
borrow() {
    ...
    drtpycontract call ${ME} --recall-nonce --pem=${PEM} --gas-limit=${GAS_LIMIT} --function="ESDTNFTTransfer" \
...
get_pool_address() {
    drtpycontract query ${ADDRESS} --function="getPoolAddress" --arguments ${ASSET} --proxy=${PROXY}
}

get_pool_address_arg() {
    drtpycontract query ${ADDRESS} --function="getPoolAddress" --arguments $1 --proxy=${PROXY}
}

get_ltv() {
    drtpycontract query ${ADDRESS} --function="getAssetLoanToValue" --arguments ${ASSET} --proxy=${PROXY}
}

get_liq_bonus() {
    drtpycontract query ${ADDRESS} --function="getAssetLiquidationBonus" --arguments ${ASSET} --proxy=${PROXY}
}
